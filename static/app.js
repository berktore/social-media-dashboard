...
fetch('/api/status').then(r => r.json()).then(data => { if (data.logged_in) showDashboard(); }).catch(function(err) { console.error('Status kontrolu basarisiz:', err.message); });
...
function loginWithCookies() {
    ...
    .then(r => r.json()).then(data => { if (data.success) showDashboard(); else { document.getElementById('login-error').textContent = 'Hata: ' + data.error; document.getElementById('login-error').classList.remove('hidden'); } }).catch(function(err) { document.getElementById('login-error').textContent = 'Baglanti hatasi'; document.getElementById('login-error').classList.remove('hidden'); });
}
...
function loadTwitterProfile(username) {
    ...
    fetch('/api/status').then(r => r.json()).then(s => {
        if (!s.twitter_ready) { document.getElementById('twitter-status').textContent = 'Twitter bagli degil'; return Promise.reject(new Error('Twitter bagli degil')); }
        return fetch(`/api/user/${username}`);
    }).then(r => {
        if (!r.ok) throw new Error('HTTP ' + r.status);
        return r.json();
    }).then(data => {
        if (data.error) { document.getElementById('twitter-status').textContent = 'Hata'; throw new Error(data.error); }
        twitterProfile = data;
        ...
        document.getElementById('twitter-status').textContent = 'Hata: ' + (err.message || err);
        console.error('Twitter yukleme basarisiz:', err.message || err);
    });
}
...
function renderTikTokAll() {
    ...
    // === Format Analysis ===
    const formatCards = [
        { key: 'short', label: 'Kisa (Ôëñ15s)', icon: 'bolt', color: '#00F2EA' },
...
function searchYouTube() {
    ...
    document.getElementById('youtube-status').textContent = 'Aran─▒yor...';
...
function loadYouTubeData(channelId) {
    ...
    // Input guncelle
    const input = document.getElementById('youtube-search-input');
    if (input && !input.value) input.value = '─░nfo Yat─▒r─▒m';
...
function renderYouTubeAll() {
    ...
    const durCards = [
        { key: 'short', label: 'Kisa (Ôëñ1 dk)', icon: 'bolt', color: '#FF0000' },
...
function loginInstagramSession() {
    const sessionid = document.getElementById('ig-sessionid').value;
    if (!sessionid) {
        document.getElementById('instagram-login-error').textContent = 'sessionid degeri gerekli';
        document.getElementById('instagram-login-error').classList.remove('hidden');
        return;
    }
    fetch('/api/instagram/login', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({sessionid}) })
    .then(r => r.json()).then(data => {
        if (data.success) {
            document.getElementById('instagram-login-modal').classList.add('hidden');
            loadInstagramData();
        } else {
            document.getElementById('instagram-login-error').textContent = 'Hata: ' + data.error;
            document.getElementById('instagram-login-error').classList.remove('hidden');
        }
    }).catch(() => {
        document.getElementById('instagram-login-error').textContent = 'Baglanti hatasi';
        document.getElementById('instagram-login-error').classList.remove('hidden');
    });
}
...
function loadInstagramData(username) {
    ...
    fetch(`/api/instagram/${username}/analytics?count=50`)
    .then(r => r.json()).then(data => {
        if (data.error) {
            statusEl.textContent = 'Hata';
            statusEl.className = 'text-label-sm font-label-sm text-error bg-error/10 px-md py-xs rounded-full border border-error/20';
            document.getElementById('instagram-profile-cards').innerHTML = `<div class="col-span-12 text-center text-error py-8">${data.error}</div>`;
            return;
        }
        instagramData = data;
        statusEl.textContent = 'Aktif';
        statusEl.className = 'text-label-sm font-label-sm text-[#E4405F] bg-[#E4405F]/10 px-md py-xs rounded-full border border-[#E4405F]/20';
        renderInstagramAll();
    }).catch(() => {
        statusEl.textContent = 'Hata';
        statusEl.className = 'text-label-sm font-label-sm text-error bg-error/10 px-md py-xs rounded-full border border-error/20';
...
function renderInstagramProfile(profile) {
    document.getElementById('instagram-profile-cards').innerHTML = `
        <div class="col-span-12 md:col-span-4 glass-card p-xl rounded-xl relative overflow-hidden group">
            <div class="absolute top-0 left-0 w-full h-1 bg-[#E4405F]"></div>
            <div class="flex items-center gap-3 mb-4">
                ${profile.profile_pic_url
                    ? `<img src="${profile.profile_pic_url}" class="w-14 h-14 rounded-full border-2 border-[#E4405F]">`
                    : `<div class="w-14 h-14 rounded-full bg-[#E4405F]/20 flex items-center justify-center"><span class="material-symbols-outlined text-[#E4405F]">camera_alt</span></div>`}
                <div>
                    <h3 class="font-body-sm font-semibold text-on-surface">${profile.full_name || profile.username}</h3>
                    <p class="font-label-sm text-on-surface-variant">@${profile.username}</p>
                </div>
            </div>
            ${profile.biography ? `<p class="font-body-sm text-on-surface-variant text-xs line-clamp-2 mb-3">${profile.biography}</p>` : ''}
            ${profile.external_url ? `<p class="text-label-sm text-primary"><a href="${profile.external_url}" target="_blank" class="hover:underline">${profile.external_url}</a></p>` : ''}
        </div>
        <div class="col-span-12 md:col-span-4 glass-card p-xl rounded-xl relative overflow-hidden group">
            <div class="absolute top-0 left-0 w-full h-1 bg-tertiary"></div>
            <div class="flex justify-between items-start mb-2">
                <span class="material-symbols-outlined text-tertiary p-1.5 bg-tertiary/10 rounded-lg">group</span>
                <span class="text-label-md text-label-sm text-on-surface-variant">${profile.media_count} post</span>
            </div>
            <h3 class="font-label-md text-label-md text-on-surface-variant uppercase tracking-widest mb-1">Takipci</h3>
            <span class="font-headline-lg text-headline-lg text-on-surface">${fmt(profile.followers)}</span>
            <p class="text-label-sm text-on-surface-variant mt-1">Takip: ${fmt(profile.following)}</p>
        </div>
        <div class="col-span-12 md:col-span-4 glass-card p-xl rounded-xl relative overflow-hidden group">
            <div class="absolute top-0 left-0 w-full h-1 bg-primary"></div>
            <div class="flex justify-between items-start mb-2">
                <span class="material-symbols-outlined text-primary p-1.5 bg-primary/10 rounded-lg">verified</span>
                <span class="material-symbols-outlined text-sm ${profile.is_verified ? 'text-[#E4405F]' : 'text-on-surface-variant'}">${profile.is_verified ? 'verified' : 'visibility_off'}</span>
            </div>
            <h3 class="font-label-md text-label-md text-on-surface-variant uppercase tracking-widest mb-1">Hesap Bilgisi</h3>
            <span class="font-headline-md text-headline-md text-on-surface">${profile.is_private ? 'Gizli Hesap' : 'Herkese Acik'}</span>
            <p class="text-label-sm text-on-surface-variant mt-1">${profile.is_verified ? 'Onayli hesap' : 'Onaysiz'}</p>
        </div>`;
}

function renderInstagramMetrics(summary) {
    if (!summary) return;
    document.getElementById('instagram-metrics').innerHTML = `
        <div class="glass-card rounded-xl p-5 relative overflow-hidden">
            <div class="absolute top-0 left-0 w-full h-1 bg-[#E4405F]"></div>
            <span class="material-symbols-outlined text-[#E4405F] mb-2">favorite</span>
            <h4 class="font-label-md text-label-md text-on-surface-variant mb-1">Toplam Begeni</h4>
            <div class="font-headline-md text-headline-md text-on-surface">${fmt(summary.total_likes)}</div>
        </div>
        <div class="glass-card rounded-xl p-5 relative overflow-hidden">
            <div class="absolute top-0 left-0 w-full h-1 bg-tertiary"></div>
            <span class="material-symbols-outlined text-tertiary mb-2">chat</span>
            <h4 class="font-label-md text-label-md text-on-surface-variant mb-1">Toplam Yorum</h4>
            <div class="font-headline-md text-headline-md text-on-surface">${fmt(summary.total_comments)}</div>
        </div>
        <div class="glass-card rounded-xl p-5 relative overflow-hidden">
            <div class="absolute top-0 left-0 w-full h-1 bg-primary"></div>
            <span class="material-symbols-outlined text-primary mb-2">bolt</span>
            <h4 class="font-label-md text-label-md text-on-surface-variant mb-1">Etkilesim Orani</h4>
            <div class="font-headline-md text-headline-md text-on-surface">%${summary.engagement_rate}</div>
        </div>
        <div class="glass-card rounded-xl p-5 relative overflow-hidden">
            <div class="absolute top-0 left-0 w-full h-1 bg-[#FF0050]"></div>
            <span class="material-symbols-outlined text-[#FF0050] mb-2">bar_chart</span>
            <h4 class="font-label-md text-label-md text-on-surface-variant mb-1">Ort. Begeni/Yorum</h4>
            <div class="font-headline-md text-headline-md text-on-surface">${fmt(summary.avg_likes)} / ${fmt(summary.avg_comments)}</div>
        </div>`;
}

function renderInstagramFormats(format_analysis) {
    if (!format_analysis) return;
    const formats = [
        { key: 'short', label: 'Fotograf', icon: 'photo', color: '#E4405F' },
        { key: 'video', label: 'Video', icon: 'video_camera_front', color: '#FF0050' },
    ];
    document.getElementById('instagram-formats').innerHTML = formats.map(f => {
        const data = format_analysis[f.key] || { count: 0, avg_likes: 0, avg_comments: 0 };
        return `
        <div class="glass-card rounded-xl p-5 relative overflow-hidden">
            <div class="absolute top-0 left-0 w-full h-1" style="background:${f.color}"></div>
            <div class="flex items-center gap-3 mb-3">
                <span class="material-symbols-outlined" style="color:${f.color}">${f.icon}</span>
                <span class="font-label-md text-label-md text-on-surface-variant">${f.label}</span>
            </div>
            <div class="font-headline-md text-headline-md text-on-surface">${data.count} post</div>
            <div class="text-label-sm text-on-surface-variant mt-1">Ort. begeni: ${fmt(data.avg_likes)}</div>
            <div class="text-label-sm text-on-surface-variant">Ort. yorum: ${fmt(data.avg_comments)}</div>
        </div>`;
    }).join('');
}

function renderInstagramRankings(best_by_likes, best_by_comments, worst_posts) {
    const renderCard = (title, icon, color, items, metric, metricLabel) => {
        if (!items || items.length === 0) return '';
        return `
        <div class="glass-card rounded-xl overflow-hidden">
            <div class="p-4 border-b border-outline-variant/30" style="background:${color}10">
                <div class="flex items-center gap-2">
                    <span class="material-symbols-outlined" style="color:${color}">${icon}</span>
                    <h3 class="font-label-md text-label-sm uppercase" style="color:${color}">${title}</h3>
                </div>
            </div>
            <div class="p-4">${items.map((p, i) => `
                <div class="flex items-start gap-3 py-3 ${i < items.length - 1 ? 'border-b border-outline-variant/20' : ''}">
                    <span class="font-headline-sm text-headline-sm text-on-surface-variant">${i + 1}</span>
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="flex items-center gap-3 mt-1 text-label-sm text-on-surface-variant">
                            <span>${fmt(p[metric])} ${metricLabel}</span>
                            <span>${fmt(p.likes)} begeni</span>
                        </div>
                    </div>
                </div>`).join('')}
            </div>
    ...
    document.getElementById('instagram-rankings').innerHTML =
        renderCard('En Cok Begeni', 'favorite', '#E4405F', best_by_likes, 'likes', 'begeni') +
        renderCard('En Cok Yorum', 'chat', '#00F2EA', best_by_comments, 'comments', 'yorum');
}

function renderInstagramHashtags(top_tags) {
    const el = document.getElementById('instagram-hashtags');
    if (!top_tags || top_tags.length === 0) {
        el.innerHTML = '<div class="col-span-5 glass-card rounded-xl p-8 text-center text-on-surface-variant">Henuz hashtag verisi yok</div>';
        return;
    }
    el.innerHTML = top_tags.map(t => `
        <div class="glass-card rounded-xl p-4 text-center">
            <div class="font-headline-sm text-headline-sm text-[#E4405F]">#${t.tag}</div>
            <div class="text-label-sm text-on-surface-variant mt-1">${t.count} post</div>
            <div class="text-label-sm text-on-surface-variant">${fmt(t.total_likes)} begeni</div>
        </div>`).join('');
}

function renderInstagramTable(posts) {
    document.getElementById('instagram-posts-count').textContent = `${posts.length} post`;
    const el = document.getElementById('instagram-posts-table');
    if (posts.length === 0) {
        el.innerHTML = '<tr><td colspan="5" class="px-4 py-8 text-center text-on-surface-variant">Post bulunamadi</td></tr>';
        return;
    }
    el.innerHTML = posts.map(p => {
        const postUrl = p.code ? `https://instagram.com/p/${p.code}/` : '#';
        const type = p.is_video ? 'Video' : 'Fotograf';
        const typeColor = p.is_video ? '#FF0050' : '#E4405F';
        return `<tr class="hover:bg-surface-container-highest transition-colors cursor-pointer" onclick="window.open('${postUrl}','_blank')">
            ...
            <td class="px-4 py-3 text-body-sm"><span class="px-2 py-1 rounded text-xs" style="color:${typeColor};background:${typeColor}15">${type}</span></td>
            <td class="px-4 py-3 text-right text-body-sm">${fmt(p.likes)}</td>
            <td class="px-4 py-3 text-right text-body-sm">${fmt(p.comments)}</td>
            <td class="px-4 py-3 text-right text-label-sm text-on-surface-variant">${formatDate(p.date_ts * 1000)}</td>
        </tr>`;
    }).join('');
}
